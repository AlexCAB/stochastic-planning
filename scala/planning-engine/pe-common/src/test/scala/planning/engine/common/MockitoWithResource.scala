...
/** Idiomatic Mockito syntax (`returns`, `was called`, ...) for ScalaTest, without `AsyncIdiomaticMockito`'s per-test
  * `MockitoSession`.
  *
  * A `MockitoSession` is bound to the thread that started it, but `UnitSpecWithResource` tests are effectively async,
  * so they can finish on another thread. The next test on the original thread then aborts the whole suite with
