...
import planning.engine.planner.mpi.actors.manager.{Manager, TestManager}
  ...
  "ManagerActor.doHandleNodeError" should:
    "terminate the manager actor after a NodeActorError is received" in newCase[CaseData]: (tn, data) =>
      ...
      async[IO]:
        ...
        manager.reportError[IO](fakeNode.api, Some(AddNodes(NodeData(conNodeData), sender.ref)), err).await
