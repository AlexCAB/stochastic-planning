...
class VisualizationService[F[_]: {Async, LoggerFactory}](topic: Topic[F, (MapInfoState[F], DcgState[F])])
...
object VisualizationService:
  def apply[F[_]: {Async, LoggerFactory}](): Resource[F, VisualizationService[F]] = Resource.eval(
    for
        topic <- Topic[F, (MapInfoState[F], DcgState[F])]
    yield new VisualizationService(topic)
  )
