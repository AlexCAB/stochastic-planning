...
private[mpi] trait Manager:
  ...
  def addNode[F[_]: Async](data: NodeData)(using Scheduler): F[MnId]
  ...
  // Used mainly for testing and debugging.
  def upsertNodesByName[F[_]: Async](data: NodeData)(using Scheduler): F[MnId]
  ...
  // Used mainly for testing and debugging.
  def addEdge[F[_]: Async](key: MeKey, sampleIds: Set[SampleId])(using Scheduler): F[MeKey]
  ...
  // Add generated by loopback learning algorithm samples command:
