...
class MapCacheStateSpec extends UnitSpecWithData with MapGraphTestData:
  ...
  "init" should:
    "create an empty MapCacheState with the given sample count" in newCase[CaseData]: (tn, data) =>
      MapCacheState.init[IO](10L).logValue(tn).asserting(_ mustEqual data.testEmptyState)

  "toQueryParams" should:
    "return a map with correct query parameters for valid state" in newCase[CaseData]: (tn, data) =>
      data.testEmptyState.toQueryParams
        .logValue(tn)
        .asserting(_ mustEqual Map(PROP.SAMPLES_COUNT -> 10L.toDbParam))

  "toCache" should:
    "add new hidden nodes in cache" in newCase[CaseData]: (_, data) =>
      async[IO]:
    ...
    "update new hidden nodes in cache" in newCase[CaseData]: (_, data) =>
    ...
    "remove prev cached nodes if number of cached bigger than maxSize" in newCase[CaseData]: (_, data) =>
      async[IO]:
  ...
  "findAndAllocateCached" should:
    "find and update position in cache" in newCase[CaseData]: (_, data) =>
      async[IO]:
