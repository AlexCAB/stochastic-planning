...
import planning.engine.map.knowledge.graph.KnowledgeGraphConfig

final case class MainConf(db: DbConf, server: ServerConf, knowledgeGraph: KnowledgeGraphConfig)
...
object MainConf:
  def formConfig[F[_]: {Sync, LoggerFactory}](appConf: Config): F[MainConf] =
    for
      ...
      knowledgeGraphConf <- KnowledgeGraphConfig.formConfig(appConf.getConfig("knowledge-graph"))
      _ <- LoggerFactory[F].getLogger.info(s"Loaded configuration: $appConf")
    yield MainConf(dbConf, serverConf, knowledgeGraphConf)
