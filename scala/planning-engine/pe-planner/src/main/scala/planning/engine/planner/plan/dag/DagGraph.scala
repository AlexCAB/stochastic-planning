...
final case class DagGraph[F[_]: MonadThrow](
  ...
  lazy val trgLinkMap: Map[SnId, Set[SnId]] = linkEdges.groupBy(_.key.trg).map((k, vs) => k -> vs.map(_.key.src).toSet)

  lazy val srcThenMap: Map[SnId, Set[SnId]] = makeSrcMap(thenEdges)
  lazy val trgThenMap: F[Map[SnId, SnId]] = makeTrgThenMap(thenEdges)

// ??? Here algorithms for building and tracing (similar to GraphStructure[F])
// ??? Context and plan together in one graph.
