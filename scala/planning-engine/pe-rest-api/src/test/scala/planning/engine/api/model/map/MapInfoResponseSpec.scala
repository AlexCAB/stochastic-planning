...
class MapInfoResponseSpec extends UnitSpecWithData with AsyncIdiomaticMockito with MapGraphTestData:
  ...
  "MapInfoResponse.fromKnowledgeGraph(...)" should:
    "create MapInfoResponse with correct values from a valid knowledge graph" in newCase[CaseData]: (tn, data) =>
      async[IO]:
        val response = MapInfoResponse.fromMapGraph(testDbName, data.mockKnowledgeGraph).logValue(tn).await
