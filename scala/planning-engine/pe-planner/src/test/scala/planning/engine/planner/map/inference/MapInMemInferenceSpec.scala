  ...
  "MapInMemInference.naiveInferActiveAbsForest" should:
    "return correct ActiveAbsDag" in newCase[CaseData]: (tn, data) =>
      ...
      async[IO]:
        ...
        val singleNode = mapInference.naiveInferActiveAbsForest(Set(c_1p1_101.id)).await
        ...
        val pathLike = mapInference.naiveInferActiveAbsForest(Set(c_1c1_111.id)).await
        ...
        val treeLike = mapInference.naiveInferActiveAbsForest(Set(c_1c2_112.id)).await
        ...
        val forestLike = mapInference.naiveInferActiveAbsForest(Set(c_1c1_111.id, c_1c2_112.id)).await
        logInfo(tn, s"\n##### Forest like: ${forestLike.repr.await}").await

        forestLike.backwordKeys.srcIds mustBe Set(
          c_1p1_101.id,
          c_1p2_102.id,
          c_1c1_111.id,
          a_2p1_201.id,
        ...
        forestLike.graph.conMnId mustBe Set(c_1c1_111.id, c_1c2_112.id)
        forestLike.graph.absMnId mustBe Set(a_2c1_211.id, a_2c2_212.id, a_3c1_311.id, a_3c2_312.id)
