...
final case class MainConf(db: DbConf, server: ServerConf, knowledgeGraph: MapConfig)
...
object MainConf:
  def formConfig[F[_]: {Sync, LoggerFactory}](appConf: Config): F[MainConf] =
    for
      ...
      knowledgeGraphConf <- MapConfig.formConfig(appConf.getConfig("knowledge-graph"))
